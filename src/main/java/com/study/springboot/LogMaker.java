...
/*  				-	macOS   - /user/daewoo/document/TmrLog/로그 파일	 */
/* 										   						  			 */
/* return 없음			                   						  			 */
/*                                         						  			 */
/* ex) int price   = 39900;				   						  			 */
...
public class LogMaker {
	...
	public void logWriter(String... param) {
		...
		//System.out.println("  line: " + new Throwable().getStackTrace()[0].getLineNumber() + " [LogMaker] logContent : " + logContent);
		
		logFileName = "tmr_" + dateYmd + "_log.txt";
		
		//System.out.println("  line: " + new Throwable().getStackTrace()[0].getLineNumber() + " [LogMaker] logFileName : " + logFileName);
		
		
		// OS 확인
		String osName = System.getProperty("os.name");
		System.out.println("  line: " + new Throwable().getStackTrace()[0].getLineNumber() + " [LogMaker] osName : " + osName);
		
		
		// 디렉토리 생성
		if(osName.contains("Windows")) {
			//System.out.println("  line: " + new Throwable().getStackTrace()[0].getLineNumber() + " [LogMaker] Windows : " + osName);
			filePath = win_mkDir();
		}else {
			//System.out.println("  line: " + new Throwable().getStackTrace()[0].getLineNumber() + " [LogMaker] macOS : " + osName);
			filePath = mac_mkDir();
		}
	...
	public String getOnlyDate() {
		...
		System.out.println("getOnlyDate : " + day);
	...
	public String getDateAndTime() {
		...
		System.out.println("getDateAndTime : " + day);
	...
	public String win_mkDir() {
		...
		if(!parent_folder.exists()) {
			parent_folder.mkdir();
			System.out.println("  line: " + new Throwable().getStackTrace()[0].getLineNumber() + " [win_mkDir] TmrLog 폴더 생성");
		}else {
			System.out.println("  line: " + new Throwable().getStackTrace()[0].getLineNumber() + " [win_mkDir] TmrLog 폴더 존재");
		}
		
		win_path += "\\";
		
		System.out.println("  line: " + new Throwable().getStackTrace()[0].getLineNumber() + " [win_mkDir] win_path : " + win_path);
	...
	public String mac_mkDir() {
		...
		String mac_path = "/user/daewoo/document/TmrLog";
		
		File parent_folder = new File(mac_path);
		
		if(!parent_folder.exists()) {
			parent_folder.mkdir();
			System.out.println("  line: " + new Throwable().getStackTrace()[0].getLineNumber() + " [mac_mkDir] TmrLog 폴더 생성");
		}else {
			System.out.println("  line: " + new Throwable().getStackTrace()[0].getLineNumber() + " [mac_mkDir] TmrLog 폴더 존재");
		}
		
		mac_path += "/";
		
		System.out.println("  line: " + new Throwable().getStackTrace()[0].getLineNumber() + " [mac_mkDir] mac_path : " + mac_path);
